...
impl RunPhase {
  ...
  fn parse_cli_args(&self, command: &Command, cli_args: Vec<String>) -> Result<(Vec<String>, std::collections::HashSet<String>, std::collections::HashMap<String, String>)> {
    ...
    let mut provided_flags = std::collections::HashSet::new();
    let mut provided_flag_values = std::collections::HashMap::new();
    let mut i = 0;
      ...
      if arg.starts_with("--") {
        ...
        if arg.contains('=') {
          ...
          if parts.len() == 2 {
            ...
            if let Some(flag) = command.flags.iter().find(|f| f.long == flag_name && f.takes_value) {
    ...
    if let Some(varargs_arg) = varargs_arg {
      let varargs_position = command.args.iter().position(|a| a.name == varargs_arg.name).unwrap();
  ...
  fn validate_required_args(&self, command: &Command, provided_args: &[String]) -> Result<()> {
    for arg in &command.args {
      if !arg.optional {
        let arg_index = command.args.iter().position(|a| a.name == arg.name).unwrap();
...
mod tests {
  use super::*;
  use crate::phases::parse::{Command, Argument, Flag};
  ...
  fn test_parse_cli_args() {
    ...
    let command = Command {
      ...
      args: vec![
        ...
        Argument { name: "arg2".to_string(), optional: true, is_varargs: false, description: None },
      ],
      flags: vec![
        Flag { short: Some('r'), long: "release".to_string(), takes_value: false, type_hint: None, description: None },
        Flag { short: None, long: "debug".to_string(), takes_value: false, type_hint: None, description: None },
      ],
      script: "echo test".to_string(),
      shebang: "#!/bin/sh".to_string(),
    };

    let cli_args = vec!["value1".to_string(), "--release".to_string(), "--debug".to_string()];
  ...
  fn test_validate_required_args() {
    ...
    let command = Command {
      ...
      group: None,
      args: vec![
        Argument { name: "required".to_string(), optional: false, is_varargs: false, description: None },
        Argument { name: "optional".to_string(), optional: true, is_varargs: false, description: None },
      ],
      flags: vec![],
      script: "echo test".to_string(),
      shebang: "#!/bin/sh".to_string(),
